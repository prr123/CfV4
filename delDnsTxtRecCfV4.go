...
//	dnsQuery := zones.ZoneListParams{Name:cloudflare.F(domStr)}
	...
	searchStr := "azultest." + domStr
	fmt.Printf("searchStr: %s\n", searchStr)

	fmt.Printf("pg array: %d\n", len(recpgar.Result))
	frec := dns.RecordResponse{}
    for _, rec := range recpgar.Result {
/*
        ...
        fmt.Printf("**** end zone ****\n")
*/
		if rec.Name == searchStr {
			frec = rec
			break
		}
    }
	if len(frec.ID) == 0 {log.Fatalf("error -- cannot find txt rec with name %s\n", searchStr)}

	fmt.Printf("**** success found txt Record ****\n")
    fmt.Printf("id:   %s\n", frec.ID)
    fmt.Printf("name: %s\n", frec.Name)
    fmt.Printf("content: %s\n", frec.Content)

	delParams := dns.RecordDeleteParams{
		ZoneID: cloudflare.F(zon.ID),
	}
	drecResp, err := client.DNS.Records.Delete(ctx, frec.ID, delParams)
	if err != nil {log.Fatalf("error -- del DnsRecs: %v\n", err)}

	fmt.Printf("resp id: %s\n", drecResp.ID)
}
